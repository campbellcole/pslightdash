...
void main()
{
	...
	float battery = 1.0;
	//if (iMouse.x > 1.0 && iMouse.y > 1.0) battery = iMouse.y / uResolution.y;
	//else battery = 0.8;
	...
	//if (abs(uv.x) < (9.0 / 16.0))
	{
		...
		else
		{
			...
			float waveVal = uv.y + sin(uv.x * 20.0 + uTime * 2.0) * 0.05 + 0.2;
			...
			// cloud
			vec2 cloudUV = uv;
			cloudUV.x = mod(cloudUV.x + uTime * 0.1, 4.0) - 2.0;
