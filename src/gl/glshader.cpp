...
namespace dash::gl {
  ...
  void GLShader::setUBool(const std::string &name, bool value) {
    this->setUInt(name, (int) value);
  }

  void GLShader::setUInt(const std::string &name, int value) {
  ...
  void GLShader::setUFloatArr(const std::string &name, GLfloat *value, int len) {
    glUniform1fv(glGetUniformLocation(ID, name.c_str()), len, value);
  }
